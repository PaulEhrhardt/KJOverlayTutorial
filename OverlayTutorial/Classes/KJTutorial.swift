...
struct KJTutorial {
  ...
  static func getMessageFrameAt(position: KJTutorialPosition, message: String, font: UIFont) -> CGRect {
    ...
    let xPos = CGFloat(max(leftPadding, (screenBounds.size.width - size.width)/2.0))
    
    var yPos = screenBounds.size.width/2.0
    switch position {
    case .top:
      yPos = topPadding
    case .bottom:
      yPos = screenBounds.size.height - size.height - bottomPadding
    }
